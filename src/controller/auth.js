const {findUser,createUser,selectUserById, verifyUser} = require('./../models/users')
...
const UsersController = {
    registerUser: async (req,res,next)=>{
        ...
        try{
            let url = `https://zealous-erin-cod.cyclic.app//auth/otp/${id}/${otp}`
    ...
    otp: async (req,res,next)=>{
        ...
        if(users.otp == otpUser){
            ...
            if(verif){
                return res.status(201).json({status:201,message:`user berhasil diverifikasi`})
            } else {
